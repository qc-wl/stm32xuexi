  ...
  if(lcddev.id==0X7789)				//2.4寸电容触摸屏
	...
	{	
		RCC->AHB1ENR|=1<<1;    		//使能PORTB时钟 
 		...
 		GPIO_Set(GPIOB,PIN12|PIN13,GPIO_MODE_IN,0,0,GPIO_PUPD_PU);					          	//PB12/PB13 设置为上拉输入
		GPIO_Set(GPIOB,PIN0,GPIO_MODE_OUT,GPIO_OTYPE_PP,GPIO_SPEED_100M,GPIO_PUPD_PU); 	//PB0设置为推挽输出
		GPIO_Set(GPIOB,PIN12,GPIO_MODE_OUT,GPIO_OTYPE_PP,GPIO_SPEED_100M,GPIO_PUPD_PU);	//PB12设置为推挽输出
 		GPIO_Set(GPIOB,PIN14,GPIO_MODE_OUT,GPIO_OTYPE_PP,GPIO_SPEED_100M,GPIO_PUPD_PU); //PB14设置推挽输出
